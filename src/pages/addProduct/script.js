...
import { updateList } from '../../components/index.js';
import { footer, nav } from '../../components/navHeader/script.js';

function ExibirMensagem(classe) {
  const msg = document.querySelector('[data_msg]');
  if (classe == "msg_enviar") {
...
function limparCampos() {
  ...
  const file = document.querySelector("[data_imgFile]");
  file.textContent = '';
...
window.onload = () => {
  ...
  const file = document.querySelector("[data_imgFile]");
  const enviar = document.querySelector('[data_send]');
